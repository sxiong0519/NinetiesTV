...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            return shows.Where(s => s.StartYear >= 1990 && 2000 > s.EndYear).ToList();
        }
        ...
        {
            return shows.OrderByDescending(s => s.EpisodeCount).ToList().FirstOrDefault();
        }
        ...
        {
            return shows.OrderByDescending(s => s.ImdbRating).Where(s => s.Genres.Contains("Drama")).ToList().FirstOrDefault();
        }
        ...
        {
            return shows.OrderBy(s => s.Name).Where( s => s.EndYear - s.StartYear > 10 && s.ImdbRating < 8).ToList().FirstOrDefault();
        }

        // 21. Return the show with the most words in the name.
        static Show WordieastName(List<Show> shows)
        {
            return shows.OrderByDescending(s => s.Name.Split(" ").Length).ToList().FirstOrDefault();
        }
        ...
        {
            return string.Join(", ", shows.Select(s => s.Name).ToArray(), 0, shows.Count - 1) + ", and " + shows.Select(s => s.Name).LastOrDefault();
        }
